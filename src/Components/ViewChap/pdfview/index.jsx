...
function PdfViewer({ pdfId, pageGet = 1 }) {
    ...
    const { pdfData, loading, error, hasMore } = useSelector((state) => state.chapter);
    ...
    return (
        ...
        >
            {pdfData?.pageContent?.length > 0 ? (
                pdfData.pageContent.map((pageContent, index) => (
                    <div key={index} className="mb-4">
                        <Document file={`data:application/pdf;base64,${pageContent}`}>
            ...
            {!hasMore && !loading && <p className="text-center text-gray-500">Đã tải hết dữ liệu PDF.</p>}
