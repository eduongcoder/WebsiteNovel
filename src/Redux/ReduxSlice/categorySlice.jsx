...
export const updateCategory = createAsyncThunk(
    ...
    async ({ idCategory, updatedCategory }, { rejectWithValue }) => {
        try {
            // Gửi PUT request với idCategory và nameCategory
            const response = await axios.put(
                `${BASE_URL}/api/category/updateCategory`,
                {
                    idCategory,
                    nameCategory: updatedCategory.nameCategory, // Đảm bảo đúng với thông tin yêu cầu
                },
            );

            // Kiểm tra mã code trong response để xử lý
            if (response.data.code === 0) {
                return response.data.result; // Nếu thành công, trả về dữ liệu đã cập nhật
            } else {
                throw new Error(response.data.message); // Nếu có lỗi từ API, ném lỗi
            }
        } catch (error) {
            return rejectWithValue(
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(updateCategory.fulfilled, (state, action) => {
                // Cập nhật danh mục trong state khi cập nhật thành công
                const index = state.categories.findIndex(
                    (category) =>
                        category.idCategory === action.payload.idCategory,
                );
                if (index !== -1) {
                    state.categories[index] = action.payload; // Cập nhật lại danh mục
                }
            })
